...
class Experiment(BaseExperiment): # must be named 'Experiment'
    ...
    async def run(self, progress_handler=None, message_handler=None):
        ...
        for i,G in enumerate(Gs):
            ...
            self.programs['2D_SLICE_MRI'].set_par('phase_GX', G[0])
            self.programs['2D_SLICE_MRI'].set_par('phase_GY', G[1])
            self.programs['2D_SLICE_MRI'].set_par('phase_GZ', G[2])
            await self.programs['2D_SLICE_MRI'].run(progress_handler=None, message_handler=message_handler)
            run_data = self.programs['2D_SLICE_MRI'].data
