...
class Experiment(BaseExperiment):  # must be named 'Experiment'
    def override(self):
        del self.par_def['recovery_time']

    # must be async or otherwise return an awaitable
    async def run(self, progress_handler=None, message_handler=None):
        self.rec_times = np.logspace(np.log10(self.par['start_rec_time']), np.log10(self.par['end_rec_time']),
                                     self.par['steps'])
        count = len(self.rec_times)
        index = 0
        self.data = None
        ...
        for rec_time in self.rec_times:
            progress_handler(index, count)
            logger.debug('running recovery time %s' % rec_time)
            self.programs['SR_CPMG'].set_par('recovery_time', rec_time)
            await self.programs['SR_CPMG'].run(message_handler=message_handler)
            run_data = self.programs['SR_CPMG'].data.view(np.complex64)
    ...
    def export_2D(self):
        ...
        t_rec = data['rec_time'][:, 0]
        t_cpmg = data['cpmg_time']
        S = getT1T2Spectrum(t_rec, t_cpmg, data['real'], data['imag'], T1, T2, alpha=1, cpmg_pts=50, exptype='SR')
    ...
    def plot_T1(self):
        ...
        result = {'data': [{
            ...
            'layout': {
                'title': 'T1',
                'xaxis': {'title': 'Recovery Time (%s)' % data['x_unit']},
