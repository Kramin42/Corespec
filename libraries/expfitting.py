...
def fit_multi_exp(x, y, N_exp=2, T2_min=10**-2, T2_max=10**2):
    spopt, spcov = curve_fit(single_exp, x, y)
    ...
    # set bounds
    mlb = [0, 1/T2_max]*N_exp  # [0, -np.inf]*N_exp
    mub = [np.inf, 1/T2_min]*N_exp  # [np.inf, 0]*N_exp
    mpopt, mpcov = curve_fit(multi_exp, x, y, p0=mp0, bounds=(mlb, mub))
