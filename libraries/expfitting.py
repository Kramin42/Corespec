...
def fit_single_exp(x, y, weights=None, T2_min=10**-4, T2_max=10**2):
    sigma = None
    if weights is not None:
        sigma = 1 / weights
    spopt, spcov = curve_fit(single_exp, x, y, sigma=sigma, p0=[0, 1], bounds=([0, 1 / T2_max], [np.inf, 1 / T2_min]))
    y_fit = single_exp(x, *spopt)
    stderr = np.sqrt(np.sum((y_fit - y) ** 2) / len(y))
    return spopt, stderr


def fit_multi_exp(x, y, weights=None, N_exp=2, T2_min=10**-4, T2_max=10**2):
    sigma = None
    if weights is not None:
        sigma = 1 / weights
    spopt, spcov = curve_fit(single_exp, x, y, sigma=sigma, p0=[0, 1], bounds=([0, 1/T2_max], [np.inf, 1/T2_min]))
